...
public class TrackRepository : BaseRepository<Track>
{
    ...
    {
        List<Track> tracks = [];
        tracks.AddRange(GetAll().Where(track => track.Genre.Equals(genre) && track.Name.Contains(name, StringComparison.CurrentCultureIgnoreCase)));
    ...
    {
        List<Track> tracks = [];
        tracks.AddRange(GetAll().Where(track => track.Artist.Equals(artist) && track.Name.Contains(name, StringComparison.CurrentCultureIgnoreCase)));
